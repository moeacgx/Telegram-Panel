...
test('私信任务使用宿主原生配置表单', () => {
  ...
  assert.match(tasksSource, /<DirectMessagingTaskConfigForm\s+v-if="isDirectMessagingTask\(editScheduledDialog\.form\.taskType\)"/)
  ...
  assert.match(tasksSource, /isDirectMessagingTask\(dialog\.form\.taskType\) && !editScheduledDraft\.value\.canSubmit/)
  ...
  assert.match(directMessagingTaskConfigFormSource, /listenerAccountId: isLiveTask\.value \? form\.listenerAccountId : null/)
  assert.match(directMessagingTaskConfigFormSource, /chats: isLiveTask\.value \? chats : \[\]/)
  assert.match(directMessagingTaskConfigFormSource, /dictionaryKey: isBatchTask\.value \? form\.dictionaryKey \|\| null : null/)
  ...
  assert.match(directMessagingTaskConfigFormSource, /messageRules: form\.contentAction === 'MessageRules' \? messageRules : cloneContentItems\(rawMessageRules\)/)
  assert.match(directMessagingTaskConfigFormSource, /forward: form\.contentAction === 'Forward' \? mergeContentItems\(rawForward,/)
  assert.match(directMessagingTaskConfigFormSource, /todo: form\.contentAction === 'Todo' \? mergeContentItems\(rawTodo,/)
  ...
  assert.match(directMessagingTaskConfigFormSource, /\.\.\.cloneRecord\(rule\.passthrough\)/)
...
test('任务复制对模块任务走通用 JSON 新建表单', () => {
  ...
  assert.match(tasksSource, /<el-dropdown-item v-if="canCopyScheduled\(row\)"[\s\S]*?@click="copyScheduledTask\(row\)"[^>]*>复制<\/el-dropdown-item>/)
...
test('任务操作支持复制到新建表单而不是立即重跑', () => {
  ...
  assert.match(tasksSource, /<el-dropdown-item[^>]*@click="copyScheduledTask\(row\)"[^>]*>复制<\/el-dropdown-item>/)
  assert.match(tasksSource, /initial-config-json="createDialog\.form\.config"/)
