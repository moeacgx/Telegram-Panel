...
私信原生表单的“不去重”开关不增加接口字段：开启时仍按现有合同提交 `dedupeDays=0`，并原样保留
`dedupeScope`；读取到 `dedupeDays=0` 时开关自动开启。关闭后恢复最后一次正数去重天数，未记录时为
`3`；`cooldownSeconds` 和 `rolling24h` 继续生效。

去重判断和历史账本均由 `pro.direct-messaging` 模块执行，宿主前端只负责承载配置并提交任务。
新建任务默认 `dedupeScope=Task`（仅当前任务去重）；需要跨任务共享去重窗口时显式选择
`dedupeScope=Global`。已有任务的配置保持原值。

模块 `1.0.2` 执行时会按目标隔离失败：账号与目标/来源组合失败会自动换发送账号，全部组合失败或
目标永久无效时只终结当前目标并继续；`uncertain` 不会自动换号重发当前目标，但不会阻断同轮其他
目标。批量任务仅在普通目标处理完后进入人工裁决，整个发送池永久不可用时才暂停整项任务。
